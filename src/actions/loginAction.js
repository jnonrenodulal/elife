    ...
    return dispatch =>{
        ...
        return fetch(url, {
            ...
            if(type===1 && json.code==='100'){
                cookie.save('userId', json.message);
                ...
                if( _subType===2){
                    dispatch(changeTab('setInfo'));  //注册成功之后，跳去设置
                }else {
...
const putInterest = (id_Array)=>{
    ...
    return dispatch =>{
        return fetch( port+'/card/interest/update/userInterest?token='+cookie.load('userId')+'&ids='+ids,{
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            }
