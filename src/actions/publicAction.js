...
import {
    ...
    SHOW_FULL_POPUP,
    SET_HEADPIC_SUCCESS,
    ADD_COLLECTED, CANCLE_COLLECTED, GET_COllECTIONLIST_SUCCESS,GET_BINDCIPHERTEXT_SUCCESS
} from './actionTypes';
...
//绑卡密文
const getBindCiphertextSuccess=text=>{
    return{
        type:GET_BINDCIPHERTEXT_SUCCESS,
        text
    }
}
...
export const getOrderCiphertext = obj =>{
    ...
    if(obj.type === 29){
        //视频 音频
        url = port + '/card/bank/scmv/pay/'+obj.cardno+'/'+obj.orderId+'?token='+cookie.load('userId');
    }else if(obj.type === 19){
        //工行服务按钮
        url = port + '/card/bank/encryption/'+obj.pickId+'/'+obj.cardNumber+'?token='+cookie.load('userId');
    }else if(obj.type === 1){
        //活动支付
        url = port + '/card/bank/encryption/pay/'+obj.cardno+'/'+obj.applyId+'?token='+cookie.load('userId');
    }else if(obj.type === 3){
        //臻品订单支付
        url = `${port}/card/bank/goods/pay/${obj.cardno}/${obj.orderId}?token=${cookie.load('userId')}`;
    }else if(obj.type==='buyCard'){
        //权益卡支付
        url = `${port}/card/bank/equity/pay/${obj.cardno}/${obj.orderId}?token=${cookie.load('userId')}`;
    }
...
//绑卡
export const getBindCiphertext=obj=>{
    let url=port+'/card/bank/encryption/register?token='+cookie.load('userId');
    return dispatch =>{
        return fetch(url)
            .then( res =>{
                console.log(res)
            ...
            .then( text =>{
                dispatch(getBindCiphertextSuccess(text));
                obj.dom.submit();
            })
            .catch( e =>{
                console.log(e)
...
const generateOrder = obj =>{
    ...
    if(obj.type === 'scmvOrder'){
        url = port + '/card/scmvOrder/create?token=' + cookie.load('userId') + '&tp=' + parseInt(new Date().getTime()/1000, 10);
    }else if(obj.type === 1){
        url = port + '/card/apply?token=' + cookie.load('userId');
    }else if(obj.type === 3){
        url=port+"/card/order/now?token="+cookie.load('userId');
    }else if(obj.type==='buyCard'){
        url=port+"/card/equityCard/create/buy/"+obj.id+"?token="+cookie.load('userId')+"&tp="+parseInt(new Date().getTime()/1000, 10)
    }
                    ...
                    if(!obj.wxPay){
                        ...
                        if(obj.type === 'scmvOrder'){
                            ...
                            hashHistory.push({
                                ...
                                query: {
                                    type: 3,
                                    orderId: json.data.orderModel.orderId
                                }
                            });
                        }else if(obj.type==='buyCard'){
                            hashHistory.push({
                        ...
                        /* 微信支付 */
                        dispatch(getWxParam({
                            orderId:obj.type===1? json.data.applyId:(obj.type===3?json.data.orderModel.orderId:json.data.id),
 ...
 *   其他： port + "/card/weixin/getRepay?orderId=" + obj.orderId + "&token=" + cookie.load('userId') + "&type=0&ipAddress=" + obj.ip;
 * */
export const getWxParam = (obj)=>{
    let url = port + "/card/weixin/getRepay?orderId=" + obj.orderId + "&token=" + cookie.load('userId') + `&type=${obj.type===1?0:(obj.type==="buyCard"?3:1)}&ipAddress=` + obj.ip;
        return fetch(url)
            .then( res=>{
                return res.json();
            })
...
const collectFn = (obj)=>{
    return dispatch =>{
        return fetch(`${port}/card/collect/item?token=${cookie.load('userId')}&itemId=${obj.itemId}&itemType=${obj.itemType}`)
            .then( res =>{
                return res.json()
            })
            .then( json =>{
...
const addCollectFn = (obj)=>{
    return dispatch =>{
        return fetch( `${port}/card/collect?token=${cookie.load('userId')}` , {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
const cancleCollectFn = (obj)=>{
    return dispatch =>{
        return fetch( `${port}/card/collect/${obj.collectId}?token=${cookie.load('userId')}`, {
            method: 'DELETE'
        } )
            .then( res =>{
                return res.json()
...
const getCollectionList = (type)=>{
    return dispatch =>{
        return fetch(`${port}/card/collect?type=${type}&token=${cookie.load('userId')}`)
            .then( res =>{
                return res.json()
            })
            .then( json =>{
