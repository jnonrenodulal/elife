...
import {
    ...
    GET_ORDER_ADDRESS,GET_CHECK_GOODS
} from './actionTypes'
...
//确认收货
const getOrderCheckGoods=(isShowCheckSuccess)=>{
    return{
        type:GET_CHECK_GOODS,
        isShowCheckSuccess
...
/*****************************/
//确认收货
const OrderCheckGoods=obj=>{
    return dispatch =>{
        return fetch( `${port}/card/order/user/check/${obj.id}?token=${cookie.load('userId')}` ,{
            method: 'PUT',
            headers: {
            ...
            isTokenExpired(json.code, function () {
                ...
                setTimeout(()=>{
                    dispatch(getOrderCheckGoods(false));
...
const getOrderList = obj =>{
    let url = '';
    if(obj.orderTab === 0){
        url = `${port}/card/apply?currentPage=${obj.page}&token=${cookie.load('userId')}`;
    }else if(obj.orderTab === 1){
        url = `${port}/card/order/v2?currentPage=${obj.page}&orderState=${obj.status}&token=${cookie.load('userId')}`;
    }else {
        url = `${port}/card/order/v2?currentPage=${obj.page}&orderState=${obj.status}&token=${cookie.load('userId')}`;
    }
...
const deleteOrder = (obj) =>{
    return dispatch =>{
        return fetch( `${port}/card/order/${obj.orderId}?token=${cookie.load('userId')}`, {
            method: 'DELETE'
        } )
            .then( res =>{
                return res.json()
...
const getOrderAddress = (obj)=>{
    return dispatch =>{
        return fetch( `${port}/card/receiver/${obj.receiveId}?token=${cookie.load('userId')}` )
            .then( res =>{
                return res.json()
            })
            .then( json=>{
...
export const fetchOrderList =(obj)=>{
    return dispatch =>{
       switch (obj.type) {
           ...
           case 6:
               return dispatch(OrderCheckGoods(obj))
           default:
               return false;
       }
    }
