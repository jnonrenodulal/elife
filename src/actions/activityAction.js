...
function fetchPostInfo(infoObj,infoStr) {
    return dispatch=>{
        return fetch( port+"/card/activityDriver/create?token="+cookie.load('userId') ,{
            method: 'POST',
            headers: {
...
const getUserActStatus = (obj)=>{
    return dispatch =>{
        return fetch( port + '/card/apply/status/'+obj.activityId+'?token='+cookie.load('userId'))
            .then(res =>{
                return res.json()
            })
            .then( json =>{
...
export const postActInfo =(infoObj,str)  =>{
    return(dispatch)=>{
        return dispatch(fetchPostInfo(infoObj,str))
    }
};
