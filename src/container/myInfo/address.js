...
class MyAddress extends Component{
    ...
    chooseChangeClick(address){
        if(this.state.itemType==3){
            hashHistory.push({
                pathname: '/goodOrder',
                query: {
                    receiverName:address.receiverName,
                    receiverPhone:address.receiverPhone,
                    addressText:address.province+address.city+address.district+address.detilAddress,
                    ...
                    stockNumber:this.props.location.query.stockNumber,
                    reMark:this.props.location.query.reMark
                }
            })
        }
    }
    ...
    render(){
        ...
        console.log("urugbr="+addressList)
        return(
                ...
                <ul>
                    {
                        addressList.map((address, index)=>{
                            return(
                                <li key={index} >
                                    <p className="receiverName" onClick={this.chooseChangeClick.bind(this,address)}>
                                        <span>{address.receiverName}</span>
                                        <span>{address.receiverPhone}</span>
                                    </p>
                                    <p className="receiverAddress" onClick={this.chooseChangeClick.bind(this,address)}>{address.province} - {address.city} - {address.district} - {address.detilAddress}
                                    </p>
                                    <p className="edit">
                                        { address.isDefault === 1 &&
