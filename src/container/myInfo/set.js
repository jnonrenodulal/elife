...
const phoneNumberWord=(tel)=>{
    return{
        str:tel.substr(0, 3) + '****' + tel.substr(7)
    }
}
...
class SetInfo extends  Component{
    ...
    render(){
        const {isDialogShow,isShowToastSuccess, isShowToastLoading,addressList} = this.props;
        return(
            <div id="userInfo">
                <Dialog type="ios" 
                ...
                <ul style={{marginTop: '0'}}>
                    ...
                    <li>
                        ...
                        <span className="last phone">{phoneNumberWord(this.props.userInfo.phone).str}</span>
                    </li>
                    <li onClick={this.handleClick.bind(this,4)} >
...

                        {
                            ...
                            addressList.map((address, index)=>{
                                if(address.isDefault===1){
                                    let addressText=address.province+address.district+address.detilAddress
                                    return (
                                        <span className="last" key={index}>{addressText}</span>
                                    )
                                }
                            })
                        }
                        {
                            addressList.length===0&&
                            <span className="last" >请添加您的地址</span>
                        }
